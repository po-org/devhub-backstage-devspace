...
const inputSchema = z.object({
  url: z.string().describe('The URL to request (absolute or /proxy/... relative)'),
...
function buildHeaders(input: InputType): Record<string,string> {
  ...
  switch(input.auth.type) {
    case 'bearer': if (input.auth.token) headers['Authorization'] = `Bearer ${input.auth.token}`; break;
    case 'basic': 
...
async function requestWithRetries(url: string, options: any, maxRetries=DEFAULT_RETRIES, verbose=false, logger?: any) {
  let attempt = 0;
...
export function createHttpAdvancedAction() {
  return createTemplateAction<InputType>({
    ...
    description: 'Enterprise-ready HTTP request with retries, polling, structured output, and proxy support',
    schema: { input: inputSchema, output: z.object({ prettyJson: z.string() }) },
    async handler(ctx) {
      const { input, logger, output, config } = ctx;
      const startTime = Date.now();
      const requestId = uuid();
      const verbose = input.verbose ?? false;

      let url = input.url;
      // Detect /proxy/ paths and prepend backend baseUrl
      if(url.startsWith('/proxy/')){
        const baseUrl = config.getString('backend.baseUrl');
        url = `${baseUrl}${url}`;
        if(verbose) logger?.info(`Using proxy URL: ${url}`);
      }
      ...
      try {
        ...
        if(input.polling?.enabled){
          ...
          do{
            const {response, retries, totalRetryMs: retryMs} = await requestWithRetries(url, requestOptions, DEFAULT_RETRIES, verbose, logger);
